...
async function createCompletion(model = MODEL_NAME, messages: any[], refreshToken: string, refConvId?: string, retryCount = 0, segmentId?: string, tools?: any[]): Promise<IStreamMessage> {
  return (async () => {
    ...
    const stream = await request('POST', `/api/chat/${convId}/completion/stream`, refreshToken, {
      ...
      headers: {
        Referer: `https://www.kimi.com/chat/${convId}`
      },
      responseType: 'stream'
    });
...
async function preSignUrl(action: string, filename: string, refreshToken: string) {
  ...
  const result = await axios.post('https://www.kimi.com/api/pre-sign-url', {
    action,
    name: filename
  }, {
    timeout: 15000,
...
async function uploadFile(fileUrl: string, refreshToken: string, refConvId?: string) {
  ...
  while (status != 'initialized' && status != 'parsed') {
    ...
    // 获取文件上传结果
    result = await axios.post('https://www.kimi.com/api/file', fileType == 'image' ? {
      type: 'image',
      file_id: fileId,
      name: filename
    } : {
  ...
  while (!parseFinish) {
    ...
    parseFinish = await new Promise(resolve => {
      axios.post('https://www.kimi.com/api/file/parse_process', {
        ids: [fileId],
        timeout: 120000
      }, {
        headers: {
...
async function getTokenLiveStatus(refreshToken: string) {
  const result = await axios.get('https://www.kimi.com/api/auth/token/refresh', {
    headers: {
      Authorization: `Bearer ${refreshToken}`,
      Referer: 'https://www.kimi.com/',
      ...FAKE_HEADERS
